...
from lib.models.modules.prototype.proto_module_max import PrototypeModule_Max_V1, PrototypeModule_Max_V2, PrototypeModule_Max_V3
from lib.models.modules.prototype.proto_module_pro import PrototypeModule_Pro_wo_BPL, PrototypeModule_Pro_wo_DMCPL
import os
...
def get_hiformer_l_configs():
    ...
    cfg.patch_size = 4
    cfg.num_classes = 2
    cfg.swin_pretrained_path = '/DATA/methods/protoseg_dev/weights/swin_tiny_patch4_window7_224.pth'
...
class HiFormer(nn.Module):
    def __init__(self, configer, config=get_hiformer_l_configs(), in_chans=3, n_classes=2):
        super().__init__()
        self.configer = configer
        self.num_classes = self.configer.get('data', 'num_classes')
        self.img_size = self.configer.get('data', 'image_size')
        self.patch_size = [4, 16]
        self.n_classes = n_classes
        self.All2Cross = All2Cross(config=config, img_size=self.img_size, in_chans=in_chans)
...
class HiFormer_PROTO_MED_MAX(nn.Module):
    def __init__(self, configer, config=get_hiformer_l_configs(), in_chans=3, n_classes=2):
        super().__init__()
        self.configer = configer
        self.num_classes = self.configer.get('data', 'num_classes')
        ...
        self.prototype_module = PrototypeModule_Max_V3(128, configer=configer, type='V3s')
        # self.prototype_module = PrototypeModule_Max_V1(128, configer=configer)

    def forward(self, x, gt_semantic_seg=None, pretrain_prototype=False, current_epoch=None, analysis_mode=False):
        xs = self.All2Cross(x)
        embeddings = [x[:, 1:] for x in xs]
        reshaped_embed = []
        for i, embed in enumerate(embeddings):
        ...
        prototype_outputs = self.prototype_module(C, gt_semantic_seg, pretrain_prototype)
        return prototype_outputs


class HiFormer_PROTO_MED_AB_WO_BPL(nn.Module):
    def __init__(self, configer, config=get_hiformer_l_configs(), in_chans=3, n_classes=2):
        super().__init__()
        self.configer = configer
        self.num_classes = self.configer.get('data', 'num_classes')
        ...
        self.prototype_module = PrototypeModule_Pro_wo_BPL(128, configer=configer)

    def forward(self, x, gt_semantic_seg=None, pretrain_prototype=False, current_epoch=None):
        xs = self.All2Cross(x)
        embeddings = [x[:, 1:] for x in xs]
        reshaped_embed = []
        for i, embed in enumerate(embeddings):
...
class HiFormer_PROTO_MED_AB_WO_DMCPL(nn.Module):
    def __init__(self, configer, config=get_hiformer_l_configs(), in_chans=3, n_classes=2):
        super().__init__()
        self.configer = configer
        self.num_classes = self.configer.get('data', 'num_classes')
        ...
        self.prototype_module = PrototypeModule_Pro_wo_DMCPL(128, configer=configer)
...
class PyramidFeatures(nn.Module):
    def __init__(self, config, img_size=224, in_channels=3):
        ...
        self.p1_pm = PatchMerging((img_size // config.patch_size, img_size // config.patch_size), config.swin_pyramid_fm[0])
        ...
        self.p2_pm = PatchMerging((img_size // config.patch_size // 2, img_size //
                                  config.patch_size // 2), config.swin_pyramid_fm[1])
...
class All2Cross(nn.Module):
    def __init__(self, config, img_size=224, in_chans=3, embed_dim=(96, 384), norm_layer=nn.LayerNorm):
        ...
        n_p1 = (img_size // config.patch_size) ** 2  # default: 3136
        n_p2 = (img_size // config.patch_size // 4) ** 2  # default: 196
