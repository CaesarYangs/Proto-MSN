...
class DTMFormerLoss(nn.Module):
    def __init__(self, configer, patch_size=8):
        super(DTMFormerLoss, self).__init__()
        self.configer = configer
        self.patch_size = patch_size
        self.img_size = self.tolist(self.configer.get('data', 'image_size'))

        self.loss_fn = DC_and_CE_loss({'batch_dice': True, 'smooth': 1e-5, 'do_bg': False}, {})
    ...
    def tolist(self, size):
        if isinstance(size, int):
            ...
            try:
                if ',' in size:
                    return [int(x.strip()) for x in size.split(',')]
                else:
                    size_int = int(size)
    ...
    def forward(self, outputs_list, label_imgs):
        outputs = outputs_list[0]
        outputs_as = outputs_list[1]

        label_imgs = Variable(label_imgs.type(torch.LongTensor)).cuda()

        label_as = rearrange(label_imgs, 'b (h p1) (w p2) -> b h w (p1 p2)', h=int(self.img_size[0]/self.patch_size),
                             w=int(self.img_size[1]/self.patch_size), p1=self.patch_size, p2=self.patch_size).max(dim=-1).values
        label_as = (label_as != 0).float()

        loss1 = self.loss_fn(outputs, label_imgs)
        loss2 = self.loss_SmoothL1(outputs_as, label_as)
...
class RobustCrossEntropyLoss(nn.Module):
    ...
    def forward(self, input: Tensor, target: Tensor) -> Tensor:
        ...
        # Log.info(f"input:{np.unique(input)}")
        # Log.info(f"target:{np.unique(target)}")

        target = self._scale_target(target[0], (input.size(2), input.size(3)))
        input = input[:, 0, :, :]  # 选择第一个类别的预测结果 # NOTE
