...
def Bilateral_voting(c_map, hori_translation, verti_translation):
    ...
    right = (torch.bmm(c_map[:, :, 4].contiguous().view(-1, row, column),
             hori_translation.view(-1, column, column))).view(batch, class_num, row, column)
    ...
    left_bottom = (torch.bmm(left_bottom.view(-1, row, column), hori_translation.transpose(3,
                   2).view(-1, column, column))).view(batch, class_num, row, column)
    right_above = (torch.bmm(verti_translation.view(-1, row, row), c_map[:, :,
                   2].contiguous().view(-1, row, column))).view(batch, class_num, row, column)
    ...
    left_above = (torch.bmm(verti_translation.view(-1, row, row), c_map[:, :,
                  0].contiguous().view(-1, row, column))).view(batch, class_num, row, column)
    left_above = (torch.bmm(left_above.view(-1, row, column), hori_translation.transpose(3,
                  2).view(-1, column, column))).view(batch, class_num, row, column)
...
class connect_loss(nn.Module):
    def __init__(self, configer, hori_translation, verti_translation, density=None, bin_wide=None):
        ...
        self.bin_wide = bin_wide
        self.density = density
        self.num_class = self.configer.get('data', 'num_classes')
