...
class FSCELossOriginal(nn.Module):
    def __init__(self, configer=None):
        super(FSCELossOriginal, self).__init__()
        self.configer = configer
        weight = None
        if self.configer.exists('loss', 'params') and 'ce_weight' in self.configer.get('loss', 'params'):
            weight = self.configer.get('loss', 'params')['ce_weight']
...
class FSCELoss(nn.Module):
    def __init__(self, configer=None):
        ...
        # # 修改后的损失函数
        self.bce_loss = nn.BCEWithLogitsLoss(weight=None, reduction='mean')  # 或者'reduction=reduction'
        Log.info(f"using CE as essientail loss")

    def forward(self, inputs, *targets, weights=None, **kwargs):
        loss = 0.0
        if self.configer.get('loss', 'loss_mode') == 'med_v1':
            target = self._scale_target(targets[0], (inputs.size(2), inputs.size(3)))
            inputs = inputs[:, 0, :, :]  # 选择第一个类别的预测结果 # NOTE
            target = (target > 0).float()  # 将target转换为二进制值
...
class FSCELoss_Release(nn.Module):
    def __init__(self, configer=None):
        super(FSCELoss_Release, self).__init__()
        self.configer = configer
        weight = None
        if self.configer.exists('loss', 'params') and 'ce_weight' in self.configer.get('loss', 'params'):
            weight = self.configer.get('loss', 'params')['ce_weight']
    ...
    def forward(self, inputs, *targets, weights=None, **kwargs):
        ...
        if isinstance(inputs, tuple) or isinstance(inputs, list):
            ...
            #! option-2
            # 分别计算每个通道的损失
            # targets feat size:torch.Size([8, 512, 512]), targets type:<class 'tuple'>
            # input feat size:torch.Size([8, 2, 512, 512]), input type:<class 'torch.Tensor'>
            #

            target = self._scale_target(targets[0], (inputs.size(2), inputs.size(3)))
            # targets[targets == ignore_index] = float('-inf')   # 将 -1 映射到一个极大的负值

            target_background = (target == 0).float()
            target_foreground = (target == 1).float()
            target_stacked = torch.stack((target_background, target_foreground), dim=1)

            # Log.info(f"targets feat size:{target.size()}, targets type:{type(target)}")
            # Log.info(f"inputs feat size:{inputs.size()}, input type:{type(inputs)}")

            # Log.info("--------")

            # Log.info(f"Targets (first element, first 5x5): {target_stacked[0, 0, 0:5, 0:5]}")
            # Log.info(f"Targets (second element, first 5x5): {target_stacked[0, 1, 0:5, 0:5]}")

            # # Log.info(f"Targets (first element, first 5x5): {targets[0][0:5, 0:5]}")
            # # Log.info(f"targets unique value:{torch.unique(targets[0])}")
            # Log.info(f"Inputs (first channel, first 5x5): {inputs[0, 0, 0:5, 0:5]}")
            # Log.info(f"Inputs (second channel, first 5x5): {inputs[0, 1, 0:5, 0:5]}")

            # 合并损失 (例如，简单相加)
            loss = self.bce_loss(inputs, target_stacked)

            #! option-3

        return loss

    def forward_legacy(self, inputs, *targets, weights=None, **kwargs):
        loss = 0.0
        if isinstance(inputs, tuple) or isinstance(inputs, list):
            if weights is None:
                weights = [1.0] * len(inputs)
            ...
            target = self._scale_target(targets[0], (inputs.size(2), inputs.size(3)))
            # loss = self.ce_loss(inputs, target)
            # inputs = inputs.squeeze(1)  # 减少一个维度，使得input的形状变为[8, 512, 256] 对维度为1的情况下使用
            inputs = inputs.select(1, 0)
            target = target.float()  # 转换target为浮点数类型
            loss = self.bce_loss(inputs, target)
...
class FSCELoss_BINARY(nn.Module):
    def __init__(self, configer=None):
        super(FSCELoss, self).__init__()
        self.configer = configer
        weight = None
    ...
    def forward(self, inputs, *targets, weights=None, **kwargs):
        ...
        if isinstance(inputs, tuple) or isinstance(inputs, list):
            ...
            target = self._scale_target(targets[0], (inputs.size(2), inputs.size(3)))
            # loss = self.ce_loss(inputs, target) # original
            # inputs = inputs.squeeze(1)  # 减少一个维度，使得input的形状变为[8, 512, 256] 对维度为1的情况下使用
            inputs = inputs.select(1, 0)
            target = target.float()  # 转换target为浮点数类型
            loss = self.bce_loss(inputs, target)
