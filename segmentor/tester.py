...
from lib.vis.palette import get_pascal_context_colors, get_cocostuff_colors, get_pascal_voc_colors, get_autonue21_colors, get_med_colors
...
class Tester(object):
    ...
    def _init_model(self):
        ...
        # TODO modify test and val judge branch
    ...
    def __relabel(self, label_map):
        ...
        Log.info(f"unique label_map value:{np.unique(label_map)}")
        Log.info(f"unique label_dist value:{np.unique(label_dst)}")
    ...
    def test(self, data_loader=None):
        ...
        elif self.configer.get('dataset') in ['med']:
            colors = get_med_colors()
        else:
        ...
        for j, data_dict in enumerate(self.test_loader):
            ...
            with torch.no_grad():
                ...
                elif self.configer.get('test', 'mode') == 'ss_test_analysis':
                    Log.info(f"ss_test_analysis")
                    outputs = self.ss_test_analysis(inputs, j, len(self.test_loader))
                ...
                Log.info(f"size of outputs:{outputs.size()}")
                Log.info("------------------------------------")
                ...
                # Log.info(f"inputs unique value is:{np.unique(inputs)}")
                # (n,c,h,w)
                for k in range(n):
                    image_id += 1
                    ori_img_size = metas[k]['ori_img_size']
                    border_size = metas[k]['border_size']
                    logits = cv2.resize(outputs[k][:border_size[1], :border_size[0]], tuple(ori_img_size), interpolation=cv2.INTER_CUBIC)

                    # Log.info(f"size of outputs before logits:{outputs.shape}")
                    ...
                    # Log.info(f"-----show logits:{logits.shape}")
                    ori_output_label_image = label_img = np.asarray(np.argmax(logits, axis=-1), dtype=np.uint8)                  # indices
                    Log.info(f"size of ori_output_label_image:{ori_output_label_image.shape}")
                    Log.info(f"label_img unique value is:{np.unique(label_img)}")
                    ...
                    if os.environ.get('save_gt_label'):
                        ...
                        color_img_ = Image.fromarray(ori_output_label_image)
                        color_img_.putpalette(colors)
        ...
        #! analysis part
        # self.seg_net.visualize_prototypes()
    ...
    def ss_test_analysis(self, inputs, j, test_len, scale=1):
        if isinstance(inputs, torch.Tensor):
            n, c, h, w = inputs.size(0), inputs.size(1), inputs.size(2), inputs.size(3)
            scaled_inputs = F.interpolate(inputs, size=(int(h * scale), int(w * scale)), mode="bilinear",
                                          align_corners=True)
            start = timeit.default_timer()
            outputs = self.seg_net.forward(scaled_inputs, analysis_mode=True)
            if j == test_len-1:
                self.seg_net.forward(scaled_inputs, analysis_mode=True)  # output analysis imgs at the final round
            torch.cuda.synchronize()
            end = timeit.default_timer()
