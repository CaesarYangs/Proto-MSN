...
class SegTask:
    ...
    # 初始化RunningScore连接的位置 - 初始化为标准RunningScore
    @staticmethod
    ...
    # 最底层性能评估计算函数 调用RunningScore的update函数进行性能参数计算
    @staticmethod
    def eval(outputs, meta, running_scores):
        """
        All use numpy to evaulation here
        
        """
        # (b,w,h,k)
        ori_target = meta['ori_target']
        # Log.info(f"content of ori_target:{ori_target}")
        # Log.info(f"size of ori_target:{ori_target.shape}")
        
        # indices = np.where(ori_target > 1)
        # values = ori_target[indices]
        # for index, value in zip(indices, values):
        #     print(f"Value at position {index}: {value}")
